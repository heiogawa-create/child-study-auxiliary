...
// 3回まちがえた後だけ使う。答えを含めた解説を許可する。
const EXPLAIN_SYSTEM_PROMPT = `あなたは小学生向けのやさしい学習サポート先生です。
子どもが同じ問題を3回まちがえたので、今回は答えとくわしい解説を教えてあげてください。

【ルール】
- まず「おしかったね、いっしょにかくにんしよう」のように短くはげます
- 正しい答えをはっきり示す
- なぜその答えになるのか、順をおってわかりやすく説明する
- まちがえた答えがある場合は、どこでまちがえやすいかにも軽くふれる
- 6歳でもわかるやさしい言葉を使い、ひらがなを多めにする
- 長くても6文程度におさめる`;
...
async function handleHint(request, env, user) {
  ...
  const { subject, question, thinking, hintLevel = 0, photo, mode, correctAnswer } = await readJson(request);
  ...
  content.push({
    ...
    text: explainMode
      ? [
        ...
        correctAnswer ? `正しい答え: ${String(correctAnswer).slice(0, 200)}` : '',
        thinking ? `子どもがまちがえて答えたもの: ${String(thinking).slice(0, 1000)}` : '',
        'この子は3回まちがえました。答えと、なぜそうなるのかのくわしい解説をやさしく教えてください。',
      ].filter(Boolean).join('\n')
      : [
        subject ? `きょうか: ${String(subject).slice(0, 30)}` : '',
        question ? `もんだい: ${String(question).slice(0, 3000)}` : 'もんだい: しゃしんをみてください',
  ...
  const response = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      max_tokens: explainMode ? 600 : 300,
      system: explainMode ? EXPLAIN_SYSTEM_PROMPT : HINT_SYSTEM_PROMPT,
